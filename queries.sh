...
echo  "$($PSQL"SELECT DISTINCT(name) FROM teams t LEFT JOIN games gm ON t.team_id = gm.winner_id LEFT JOIN games gs ON t.team_id = gs.opponent_id WHERE (gm.year = 2014 AND gm.round = 'Eighth-Final') OR (gs.year=2014 AND gs.round='Eighth-Final') ORDER BY name")"
